...
@section('title', 'Persetujuan Seminar Komprehensif Mahasiswa')

@section('content')
<div class="container">
    <h1>Persetujuan Seminar Komprehensif Mahasiswa</h1>
    <hr>
    ...
    <table class="table" id="semkomTable">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach($seminarProposals as $index => $sempro)
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ asset('uploads/seminar_komprehensif/' . $sempro->transkrip_nilai) }}" target="_blank">
                            <i class="fas fa-file fa-2x text-success"></i>
                        </a>
                    </td>
                    <td class="text-center">
                        <a href="{{ asset('uploads/seminar_komprehensif/' . $sempro->Kartu_Bimbingan) }}" target="_blank">
                            <i class="fas fa-file fa-2x text-success"></i>
                        </a>
                    </td>
                    <td class="text-center">
                        <a href="{{ asset('uploads/seminar_komprehensif/' . $sempro->sertifikat_pkl) }}" target="_blank">
                            <i class="fas fa-file fa-2x text-success"></i>
                        </a>
                    </td>
                    <td class="text-center">
                        <a href="{{ asset('uploads/seminar_komprehensif/' . $sempro->KRS) }}" target="_blank">
                            <i class="fas fa-file fa-2x text-success"></i>
                        </a>
                    </td>
                        <td class="text-center">
                    <div class="d-inline-flex align-items-center ">
                        <form action="{{ route('dosen.approve_semkom', $sempro->id) }}" method="POST" class="approval-form" style="display:inline; margin-right: 5px  ">
                            @csrf
                        ...
                        <form action="{{ route('dosen.reject_semkom', $sempro->id) }}" method="POST" class="approval-form" style="display:inline;">
                            @csrf
