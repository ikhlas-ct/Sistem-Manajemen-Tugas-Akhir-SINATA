...
@section('title', 'Atur Jadwal Seminar Proposal')

@section('content')
<div class="container mt-5">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h2>Atur Jadwal Seminar Proposal</h2>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('prodi.setuju.sempro', $seminarProposal->id) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="dosen_penguji_1_id" id="dosen_penguji_1_id" class="form-select" required>
                        ...
                        @foreach($dosens as $dosen)
                            @php
                                $selected1 = ($dosen->id == $seminarProposal->dosen_penguji_1_id) ? 'selected' : '';
                            @endphp
                ...
                <div class="mb-3">
                    ...
                    <select name="dosen_penguji_2_id" id="dosen_penguji_2_id" class="form-select" required>
                        ...
                        @foreach($dosens as $dosen)
                            @php
                                $selected2 = ($dosen->id == $seminarProposal->dosen_penguji_2_id) ? 'selected' : '';
                            @endphp
                ...
                <div class="mb-3">
                    ...
                    <input type="datetime-local" name="tanggal_waktu" id="tanggal_waktu" class="form-control" required min="{{ date('Y-m-d\TH:i') }}"
                        value="{{ \Carbon\Carbon::parse($seminarProposal->tanggal_waktu)->format('Y-m-d\TH:i') }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="ruangan_id" id="ruangan_id" class="form-select" required>
                        ...
                        @foreach($ruangans as $ruangan)
                            @php
                                $selected = ($ruangan->id == $seminarProposal->ruangan_id) ? 'selected' : '';
                            @endphp
...
<script>
    ...
    $(document).ready(function() {
        validateDosenPengujiSelections({{ optional($seminarProposal->mahasiswaBimbingan->dosenpembimbing)->dosen_id }});
    });
